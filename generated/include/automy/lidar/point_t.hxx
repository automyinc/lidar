
// AUTO GENERATED by vnxcppcodegen

#ifndef INCLUDE_automy_lidar_point_t_HXX_
#define INCLUDE_automy_lidar_point_t_HXX_

#include <vnx/Type.h>
#include <automy/lidar/package.hxx>
#include <automy/math/Vector3d.hpp>


namespace automy {
namespace lidar {

struct AUTOMY_LIDAR_EXPORT point_t {
	
	
	int64_t time = 0;
	::automy::math::Vector3d position;
	vnx::float32_t distance = 0;
	vnx::float32_t intensity = 0;
	
	static const vnx::Hash64 VNX_TYPE_HASH;
	static const vnx::Hash64 VNX_CODE_HASH;
	
	static constexpr uint64_t VNX_TYPE_ID = 0x38fdcb2273263568ull;
	
	point_t() {}
	
	vnx::Hash64 get_type_hash() const;
	std::string get_type_name() const;
	const vnx::TypeCode* get_type_code() const;
	
	static std::shared_ptr<point_t> create();
	std::shared_ptr<point_t> clone() const;
	
	void read(vnx::TypeInput& _in, const vnx::TypeCode* _type_code, const uint16_t* _code);
	void write(vnx::TypeOutput& _out, const vnx::TypeCode* _type_code, const uint16_t* _code) const;
	
	void read(std::istream& _in);
	void write(std::ostream& _out) const;
	
	template<typename T>
	void accept_generic(T& _visitor) const;
	void accept(vnx::Visitor& _visitor) const;
	
	vnx::Object to_object() const;
	void from_object(const vnx::Object& object);
	
	vnx::Variant get_field(const std::string& name) const;
	void set_field(const std::string& name, const vnx::Variant& value);
	
	friend std::ostream& operator<<(std::ostream& _out, const point_t& _value);
	friend std::istream& operator>>(std::istream& _in, point_t& _value);
	
	static const vnx::TypeCode* static_get_type_code();
	static std::shared_ptr<vnx::TypeCode> static_create_type_code();
	
};

template<typename T>
void point_t::accept_generic(T& _visitor) const {
	_visitor.template type_begin<point_t>(4);
	_visitor.type_field("time", 0); _visitor.accept(time);
	_visitor.type_field("position", 1); _visitor.accept(position);
	_visitor.type_field("distance", 2); _visitor.accept(distance);
	_visitor.type_field("intensity", 3); _visitor.accept(intensity);
	_visitor.template type_end<point_t>(4);
}


} // namespace automy
} // namespace lidar


namespace vnx {

template<>
struct is_equivalent<::automy::lidar::point_t> {
	bool operator()(const uint16_t* code, const TypeCode* type_code);
};

} // vnx

#endif // INCLUDE_automy_lidar_point_t_HXX_
